...
form1.onsubmit = function(e){
    e.preventDefault()
    if(numInpGroups.value == ""||numOutGroups.value == "" || numInpGroups.value <1||numOutGroups.value < 1){
        return 0;
    }
...
form2.onsubmit = function (e) {
    ...
    for(let i =0;i<numInpGroups.value;i++){
        ...
        for (let j = 0; j < groupsLengthInp[i].value; j++) {
            let label = document.createElement("label")
            label.textContent = `Enter The Value Of Element Number ${j+1} At Group Number ${i+1}`
